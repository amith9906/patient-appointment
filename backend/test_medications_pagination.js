...
function makeReq(page) {
  return new Promise((resolve, reject) => {
    const req = http.request({
      hostname: '127.0.0.1',
      port: 5010,
      path: `/api/medications?page=${page}&limit=2`,
      method: 'GET',
...
async function run() {
  const p1 = await makeReq(1);
  const p2 = await makeReq(2);

  console.log('=== RAW JSON RESPONSE: GET /api/medications?page=1&limit=2 ===');
  console.log(JSON.stringify(p1.data, null, 2));

  console.log('\n=== RAW JSON RESPONSE: GET /api/medications?page=2&limit=2 ===');
