...
export default function PatientDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      api.get('/patients/me'),
      api.get('/patients/me/appointments?status=scheduled'),
      api.get('/patients/me/reports'),
    ]).then(([p, a, r]) => {
      setPatient(p.data);
      setAppointments(a.data.slice(0, 3));
  ...
  const statusColor = { scheduled: 'bg-blue-100 text-blue-700', confirmed: 'bg-green-100 text-green-700', completed: 'bg-gray-100 text-gray-600', cancelled: 'bg-red-100 text-red-700' };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-800 rounded-2xl p-6 text-white">
        ...
        {patient && (
          <div className="mt-4 flex gap-6 text-sm">
            ...
            {patient.allergies && <span>⚠️ Allergies on file</span>}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { to: '/patient-portal/book', icon: '📅', label: 'Book Appointment', color: 'bg-blue-50 border-blue-200 text-blue-700' },
          { to: '/patient-portal/appointments', icon: '🗓️', label: 'My Appointments', color: 'bg-green-50 border-green-200 text-green-700' },
          { to: '/patient-portal/reports', icon: '📋', label: 'My Reports', color: 'bg-purple-50 border-purple-200 text-purple-700' },
          { to: '/patient-portal/prescriptions', icon: '💊', label: 'Prescriptions', color: 'bg-orange-50 border-orange-200 text-orange-700' },
        ].map(item => (
            ...
            <div className="space-y-3">
              {reports.map(r => (
                <div key={r.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                  <div className="text-2xl">{r.mimeType?.includes('pdf') ? '📄' : '🖼️'}</div>
                  <div className="flex-1">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          ...
          {patient.allergies && (
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <span className="text-yellow-700 text-sm">⚠️ <strong>Allergies:</strong> {patient.allergies}</span>
            </div>
          )}
        </div>
      )}
