...
const STATUS_COLORS = { scheduled: 'bg-blue-100 text-blue-700', confirmed: 'bg-green-100 text-green-700', in_progress: 'bg-yellow-100 text-yellow-700', completed: 'bg-gray-100 text-gray-600', cancelled: 'bg-red-100 text-red-600' };
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setStats({
        ...
        pending: allA.data.filter(a => a.status === 'scheduled').length,
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Appointments', value: stats.total, icon: '📅', color: 'text-blue-600 bg-blue-50' },
          { label: "Today's Schedule", value: stats.today, icon: '🗓️', color: 'text-teal-600 bg-teal-50' },
          { label: 'Pending', value: stats.pending, icon: '⏳', color: 'text-orange-600 bg-orange-50' },
          { label: 'Completed', value: stats.completed, icon: '✅', color: 'text-green-600 bg-green-50' },
        ].map(s => (
